...
wx.ready(function () {
    ...
    $("#infoWindow #infoExit a span").on("click", () => {
        wx.openLocation({
            latitude: selfTargetLat, // 纬度，浮点数，范围为90 ~ -90
            longitude: selfTargetLng, // 经度，浮点数，范围为180 ~ -180。
            name: selfTargetName, // 位置名
            address: selfTargetAddress, // 地址详情说明
    ...
    // 选取图片 拉起
    $("#clockInLists .clockIn_button").on('click', () => {
        ...
        wx.getLocation({
            ...
            success: function (res) {
                ...
                let a = getFlatternDistance(Number(res.latitude), Number(res.longitude), selfTargetLat, selfTargetLng);
                // let a = getFlatternDistance(Number(res.latitude), Number(res.longitude), 30.54152, 104.063886);
                if (a < 1000) {
