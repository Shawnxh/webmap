 ...
 */
var graphicTotalPage,
    graphicCurrentPageSize,
    graphicPageNum = 1,
    graphicPageSize = 3;
var graphic_data; // 进入图文列表
...
$("#graphicList .graphicList_back").on("click", function () {
  $("#graphicList").hide();
}); // 进入图文详情

function entrygraphicDetail() {
  $("#graphicList .main .list .item .cover").unbind("click");
  $("#graphicList .main .list .item .cover").on("click", function (e) {
    var temp = selectGraphic($(e.currentTarget).attr("data_id"));
...
$("#graphicDetail .graphicDetail_exit").on("click", function () {
  // 退出详情且暂停详情页面内的video
