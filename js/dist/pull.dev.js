...
var mescroll_clockInList = new MeScroll("mescroll_clockInList", {
  ...
  up: {
    ...
    noMoreSize: 3,
    //如果列表已无数据,可设置列表的总数量要大于10才显示无更多数据;
...
var mescroll_commentsList = new MeScroll("mescroll_commentsList", {
  //第一个参数"mescroll"对应上面布局结构div的id (1.3.5版本支持传入dom对象)
  //如果您的下拉刷新是重置列表数据,那么down完全可以不用配置,具体用法参考第一个基础案例
  //解析: down.callback默认调用mescroll.resetUpScroll(),而resetUpScroll会将page.num=1,再触发up.callback
  down: {
  ...
  up: {
    auto: false,
    isBounce: false,
    //如果为true，某些ios手机会有bug
    callback: commentsList_upCallback,
    //上拉加载的回调
    //以下是一些常用的配置,当然不写也可以的.
    page: {
      num: 1,
...
var mescroll_graphicList = new MeScroll("mescroll_graphicList", {
  //第一个参数"mescroll"对应上面布局结构div的id (1.3.5版本支持传入dom对象)
  //如果您的下拉刷新是重置列表数据,那么down完全可以不用配置,具体用法参考第一个基础案例
  //解析: down.callback默认调用mescroll.resetUpScroll(),而resetUpScroll会将page.num=1,再触发up.callback
  down: {
  ...
  up: {
    ...
    isBounce: false,
    //如果为true，某些ios手机会有bug
    callback: graphicList_upCallback,
    //上拉加载的回调
    //以下是一些常用的配置,当然不写也可以的.
    page: {
      num: 1,
...
function clockIn_downCallback() {
  setTimeout(function () {
    var parent = $("#clockInLists .main .lists");
    parent.empty();
    clockInPageNum = 1;
    $.ajax({
      type: "post",
...
function graphicList_downCallback() {
  setTimeout(function () {
    ...
    $.ajax({
      ...
      async: false,
      beforeSend: function beforeSend(request) {// request.setRequestHeader("token", sessionStorage.getItem("token"));
      },
      success: function success(res) {
  ...
  setTimeout(function () {
    ...
    $.ajax({
      ...
      success: function success(res) {
        ...
        mescroll_graphicList.endByPage(graphicCurrentPageSize, graphicTotalPage); // mescroll_graphicList.endSuccess(graphicCurrentPageSize, res.data.hasNextPage);
