...
var map = null,
    ...
    marker = null,
    marker_current = null,
    imageTileLayer = null; // 存储用户打开信息窗口的方式
...
window.onload = function () {
  // 页面打开时定位当前位置
  geolocation.getLocation(getPosition, failSelfPosition, options); // 获取地图数据

  getData(); // 地址载入

  initMap(); // 绑定项目内功能点事件

  bindEvent(); // $(document).on('mousewheel', () => {
  //     console.log(map.getZoom());
  // })
};

function initMap() {
  var drawContainer = document.getElementById('mapContainer'); // 项目中心点

  var center = new TMap.LatLng(30.46267, 103.930809); //初始化地图
...
function createMarker() {
  ...
  marker.on("click", function (evt) {
    ...
    selfTargetAddress = evt.geometry.address; // 移动地图中心
...
function getData() {
  ...
  $.ajax({
    ...
    success: function success(res) {
      ...
      var parent = $("#buttonContainer .center");
      parent.empty();
      ...
      $("#buttonContainer .button button").each(function (index, ele) {
        var icon = $(ele).attr("icon");
        var iconButton = $(ele).attr("iconButton"); // 为底部生成默认默认背景图片的样式
  ...
  if ($("#buttonContainer .button button").length > 0) {
    $.ajax({
      ...
      async: false,
      success: function success(res) {
        marker_data = res.data.map(function (item) {
          var temporary_object = new Object();
...
function bindEvent() {
  // 关闭信息窗口
  $("#infoWindow .head .close").on("click", function () {
    $("#infoWindow").hide();
  ...
  $("#sketchcontainer .close").on("click", function () {
  ...
  $("#buttonContainer .button button").on("click", function () {
    ...
    $(ele).on("click", function (e) {
      $.ajax({
        ...
        success: function success(res) {
          ...
          assignTourDirectory(directory); // 为所有创建的目录注册 查看点击事件

          $("#tour_directory .nav li span:last-child").on("click", function (e) {
            viewDetails(e);
            from = "view";
          });
        },
      ...
      if ($("#tour_directory").css("display") == "none") {
        $("#buttonContainer .center .button button").eq(index).css({
          "background-image": "url(" + $(e.currentTarget).attr('iconButton') + ")"
        }).parent().siblings().children("button").each(function (index, ele) {
          $(ele).css({
            "background-image": "url(" + $(ele).attr('icon') + ")"
          });
        });
        $("#tour_directory").show();
        drag = index;
        ...
        $("#buttonContainer .center .button button").eq(index).css({
          "background-image": "url(" + $(e.currentTarget).attr('icon') + ")"
        });
        $("#tour_directory").hide();
      } else if ($("#tour_directory").css("display") == "block" && drag != index) {
        $("#buttonContainer .center .button button").eq(index).css({
  ...
  $("#moreUnfold .button .close_icon").on("click", function () {
    $("#moreUnfold").fadeOut();
  }); // 打开分享页面

  $("#moreUnfold .button .share_icon").on("click", function () {
    $("#moreUnfold").hide();
    $("#shareInstructionPage").fadeIn();
  }); // 关闭分享页面

  $("#shareInstructionPage .okay").on("click", function () {
    $("#shareInstructionPage").hide();
    $("#moreUnfold").fadeIn();
  }); // 路线规划

  $("#infoWindow .head .right .pathPlanning").on("click", function (e) {
  ...
  $("#infoWindow #infoExit >span").on("click", function () {
    // 删除路线规划
    polylineLayer.remove("pl_1"); // 还原缩放

    map.zoomTo(15); // 还原起始位置 => map中心设置到起点
...
function assignTourDirectory(array) {
  var parent = $("#tour_directory .nav");
  parent.empty();

  for (var i = 0; i < array.length; i++) {
    var a = $("<li class='primary_title' byid=" + array[i].byid + "><span class='title'>" + array[i].title + "</span><span class='detail'>查看</span></li>");
    parent.append(a);
  }
} // 展示弹窗+填充弹窗
...
function viewDetails(e) {
  // 关闭导览目录页面
  $("#tour_directory").hide(); // 通过父节点上的byid匹配当前标记点集合中该标记点数据

  navi_data = selectNavi(e.target.parentNode.getAttribute('byid')); // 设置目标位置的经纬度
  ...
  assignInfoWindow("directory", navi_data); // 移动地图中心
...
function cb(ret) {
  ...
  var coors = ret.result.routes[0].polyline,
      pl = []; //坐标解压（返回的点串坐标，通过前向差分进行压缩，因此需要解压）
...
function display_polyline(pl) {
  ...
  map.zoomTo(13); // map中心设置到起点

  map.panTo(new TMap.LatLng(selfCurrentLat, selfCurrentLng)); // 路线显示完 => 显示 退出 导航
