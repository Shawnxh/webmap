 ...
 */
// 语音助手点击事件
$("#infoWindow #infoContainer .bottom .voiceAssistant").on("click", function (e) {
  assignVoice(e);
  $("#infoWindow").hide();
  $("#voice").show();
}); // 关闭语音助手 回到 信息弹窗

$("#voice .head .close").on("click", function () {
  $("#voice .voice_content .back").click();
  $("#voice").hide();
  $("#infoWindow").show();
}); // 退出audio,且还原到目录打开页面 + 暂停播放

$("#voice .voice_content .back").on("click", function () {
...
function assignVoice(e) {
  ...
  var temp = selectVoice(e.currentTarget.getAttribute("byid"));
  var parent = $("#voice .voice_directory");
  parent.empty();

  for (var i = 0; i < temp.length; i++) {
    var a = $("<div class=\"title\" file=" + temp[i].audio + " data_id=" + temp[i].id + ">" + temp[i].name + "</div>");
    parent.append(a);
  } // 生成目录 => 注册语音目录事件


  $("#voice .voice_directory .title").unbind("click");
  $("#voice .voice_directory .title").on("click", function (e) {
    $("#voice .voice_content audio").attr("src", $(e.currentTarget).attr('file'));

    for (var _i = 0; _i < temp.length; _i++) {
      if ($(e.currentTarget).attr('data_id') == temp[_i].id) {
        $("#voice .voice_content .text").html(temp[_i].content);
      }
    }
