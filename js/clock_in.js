...
var clockInTotalPage, clockInCurrentPageSize, clockInPageNum = 1, clockInPageSize = 3;

// 进入打卡列表
$("#infoWindow .bottom .clockIn").on("click", () => {
    if (!sessionStorage.getItem('token') || !token) {
        window.location.href = baseUrl + '/api/user/login' + '?redirectUrl=' + encodeUrl;
    } else {
        $("#clockInLists .main .lists").empty();
        // 填充打卡列表数据
        assignClockInHeader();
...
// 退出打卡列表
$("#clockInLists .head .close").on("click", () => {
    $("#clockInLists").hide();
    $("#infoWindow").show();
});
...
function clockInListsThunmUpStatus() {
    ...
    $("#clockInLists .main .item .cover").unbind('click');
    // 打卡详情
...
// ==================================打卡信息发布页============================================
// 发布页 => 打卡列表
$("#clockInFn .bottom_original .back").on('click', () => {
    $("#clockInFn").hide();
...
// 图片海报返回
$("#clockInFn .downImgContainer .backToShare").on('click', () => {
    $("#clockInFn .downImgContainer").hide();
    // 生成图片置空
    $("#clockInFn .downImgContainer img").attr("src", "");
    // 回到打卡列表
